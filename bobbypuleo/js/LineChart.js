class LineChart {
	constructor(opts) {
		...
		d3.csv(this.data, data => {
			this.data = data;
			if (this.dropdown) {
				this.metrics = [...new Set(data.map(d => d.skater))];
	...
	draw(currMetric) {
		...
		// set current data
		let currData = this.data.filter(d => d.skater == currMetric);
		...
		const xAxisCall = d3.axisBottom(x).ticks(3).tickFormat(d3.format("d"));
