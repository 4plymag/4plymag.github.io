var barMargin = { top: 10, right: window.innerWidth < 1000 ? 5 : window.innerWidth / 8, bottom: 100, left: window.innerWidth / 6 }
var barWidth = (window.innerWidth * 0.8) - barMargin.right - barMargin.left;
const barHeight = window.innerWidth > 1000 ? window.innerHeight * 0.8 : window.innerHeight * 0.6;
...
function drawog() {
    ...
    barWidth = (window.innerWidth * 0.9) - barMargin.right - barMargin.left;
    ...
    d3.csv("detaildf.csv", function(data) {
        let trickByObstacle = d3.nest()
            ...
            .entries(data)
trickByObstacle.sort(function(a, b) {
...
barData.sort(function(a, b) {
    return b.value - a.value;
        ...
        document.getElementById("bar-header").innerHTML = 'Top Tricks by Obstacle';
        // Add X axis
        let x = d3.scaleLinear()
            .domain([0, (d3.max(trickByObstacle, function(d) { return +d.value; }))])
            .range([0, barWidth])

        let tickValues = d3.range(0, (d3.max(trickByObstacle, function(d) { return +d.value; })), (d3.max(trickByObstacle, function(d) { return +d.value; })/10));
let xAxis = d3.axisBottom(x).tickValues(tickValues).tickSize(0);
        ...
        d3.selectAll("rect")
            .on('click', function() {
                ...
                document.getElementById("bar-header").innerHTML = headerText;
                d3.selectAll("rect")
                    .remove();
