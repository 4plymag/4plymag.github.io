class LineChart {
	constructor(opts) {
		...
		d3.csv(this.data, data => {
			this.data = data;
			if (this.dropdown) {
				this.metrics = [...new Set(data.map(d => d.trick))];
	...
	draw(currMetric) {
		...
		// set current data
		let currData = this.data.filter(d => d.trick == currMetric);

		const x = d3.scaleLinear()
			.domain([d3.min(currData, d => d[this.xValue]) - .2,
		...
		const xAxisCall = d3.axisBottom(x).ticks(17).tickFormat(d3.format("d"));
		this.xAxisGroup.transition().duration(500).call(xAxisCall)

		const yAxisCall = d3.axisLeft(y).ticks(7).tickFormat(d3.format("d"));
		...
		circles.transition(t)
			.attr('cx', d => x(d[that.xValue]))
			.attr('cy', d => y(+d[that.yValue]))
			.attr('r', window.innerWidth < 1000 ? 6 : 10)
			.attr('stroke-width', 3)
			.attr('stroke', 'white')
